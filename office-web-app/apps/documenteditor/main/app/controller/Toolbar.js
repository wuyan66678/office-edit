            ...
            this.onApiEndAddShape = function() {
                ...
                if (this.toolbar.btnInsertText.pressed)
                    this.toolbar.btnInsertText.toggle(false, true);
								this.api._doRecordCollaboration(this._addShapeType || '插入形状');
...
//            Common.NotificationCenter.on('menu:afterkeydown', _.bind(this.onAfterKeydownMenu, this));
        ...
        onSearchDlgHide: function() {
            ...
            this.toolbar.btnReplace.toggle(false, true);
            this.api.asc_selectSearchingResults(false);
            $(this.toolbar.btnSearch.el).blur();
            $(this.toolbar.btnReplace.el).blur();
        ...
        attachUIEvents: function(toolbar) {
            ...
            toolbar.btnCut.on('click',                                  _.bind(this.onCut, this));
            ...
            toolbar.btnReplace.on('click',                              _.bind(this.onSearch, this));
        ...
        onSearch: function(btn,s){
            ...
            var mode = btn ? btn.options.action : s
        ...
        onApiFocusObject: function(selectedObjects) {
            ...
            if (!(paragraph_locked || header_locked)) {
                ...
                //分割线隐藏后序号变动
                toolbar.btnContentControls.menu.items[7].setDisabled(!in_control || lock_type==Asc.c_oAscSdtLockType.SdtContentLocked || lock_type==Asc.c_oAscSdtLockType.SdtLocked || if_form);
                toolbar.btnContentControls.menu.items[8].setDisabled(!in_control || if_form);
            }
        ...
        onCopyPaste: function(copy, e) {
            ...
            if (me.api) {
                var res = (copy) ? me.api.Copy() : me.api.Paste();
								if(copy) {
									me.api._doRecordCollaboration('设置格式：应用格式刷22')
								} else {
									me.api._doRecordCollaboration('粘贴')
								}
                // if (!res) {
        ...
        onIncrease: function(e) {
            if (this.api) {
							this.api.FontSizeIn();
							this.api._doRecordCollaboration(`设置格式：增大字号`)
						}
        ...
        onDecrease: function(e) {
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置格式：减小字号`)
						}
        ...
        onBold: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置格式：字体：加粗')
							} else {
								this.api._doRecordCollaboration('设置格式：字体：取消加粗')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onItalic: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置格式：字体：倾斜')
							} else {
								this.api._doRecordCollaboration('设置格式：字体：取消倾斜')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onUnderline: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置格式：字体：下划线')
							} else {
								this.api._doRecordCollaboration('设置格式：字体：取消下划线')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onStrikeout: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置格式：字体：删除线')
							} else {
								this.api._doRecordCollaboration('设置格式：字体：取消删除线')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onSuperscript: function(btn, e) {
            if (!this.toolbar.btnSubscript.pressed) {
                ...
                if (this.api) {
									...
									if (btn.pressed) {
										this.api._doRecordCollaboration('设置格式：字体：上标')
									} else {
										this.api._doRecordCollaboration('设置格式：字体：取消上标')
									}
								}
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onSubscript: function(btn, e) {
            if (!this.toolbar.btnSuperscript.pressed) {
                ...
                if (this.api) {
									...
									if (btn.pressed) {
										this.api._doRecordCollaboration('设置格式：字体：下标')
									} else {
										this.api._doRecordCollaboration('设置格式：字体：取消下标')
									}
								}
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onDecOffset: function(btn, e) {
            if (this.api){
							...
							this.api._doRecordCollaboration('设置格式：段落：减少缩进')
						}
        ...
        onIncOffset: function(btn, e) {
            if (this.api) {
							...
							this.api._doRecordCollaboration('设置格式：段落：增加缩进')
						}
        ...
        onHorizontalAlign: function(type, btn, e) {
            ...
            if (this.api) {
							...
							this.api._doRecordCollaboration(actionContent)
        ...
        onFontNameSelect: function(combo, record) {
            if (this.api) {
                if (record.isNewFont) {
                    ...
                    this.api.put_TextPrFontName(record.name);
                    Common.component.Analytics.trackEvent('ToolBar', 'Font Name');
										this.api._doRecordCollaboration(`设置格式：字体：${record.name}`)
                }
            }
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        },
        ...
        onFontSizeSelect: function(combo, record) {
            ...
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置格式：字号：${record.value}pt`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
            Common.component.Analytics.trackEvent('ToolBar', 'Font Size');
        },
        ...
        onChangeCase: function(menu, item, e) {
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置格式：大小写：${item.caption}`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        },
            ...
            if (this.api) {
							...
							if(record.data) {
								switch(record.data.type) {
									case 0:
										actionContent = '应用项目符号';
										break;
									case 1:
										actionContent = '应用编号';
										break;
								}
								if(record.data.subtype == -1) {
									actionContent += '：无';
								}
							} else {
								if(btn.options && btn.options.hint) {
									actionContent = `应用${btn.options.hint}`
									if(record.attributes && record.attributes.tip === '无') {
										actionContent += '：无';
									}
								}
							}
        ...
        onChangeLevelClick: function(type, menu, item) {
            if (this.api) {
                this.api.asc_SetNumberingLvl(item.options.level);
								this.api._doRecordCollaboration('应用符号')
            }
        ...
        onLineSpaceToggle: function(menu, item, state, e) {
            if (!!state) {
                ...
                if (this.api) {
									...
									this.api._doRecordCollaboration(`设置格式：段线间距：${item.value}`)
								}
        ...
        onTablePickerSelect: function(picker, columns, rows, e) {
            if (this.api) {
                ...
                this.api.put_Table(columns, rows);
								this.api._doRecordCollaboration('插入表格')
            }
                        ...
                        if (result == 'ok') {
                            if (me.api) {
                                ...
                                me.api.put_Table(value.columns, value.rows);
																me.api._doRecordCollaboration('插入表格')
                            }
        ...
        onClearStyleClick: function(btn, e) {
            if (this.api) {
							...
							this.api._doRecordCollaboration('设置格式：清除样式')
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        },
            ...
            switch (item.value) {
                ...
                case 5:
                    ...
                    break;
						...
						if(item.value != 5) {
							window['_PostActionContent'](`设置行号：${item.caption}`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        },
            ...
            if (chart) {
                ...
                if (isCombo && chart.getSeries().length<2) {
                    ...
                    chart.changeType(type);
										me.api._doRecordCollaboration('插入图表')
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
            } else {
        ...
        onInsertPageNumberClick: function(picker, item, record, e) {
            if (this.api) {
							...
							let actionCont = ''
							if(record.attributes.data.type == 1) {
								actionCont = '在页眉插入页码'
							} else {
								actionCont = '在页眉插入页码'
							}
							window['_PostActionContent'] && window['_PostActionContent'](actionCont)
						}
        ...
        onPageNumCurrentPosClick: function(item, e) {
            if (this.api) {
							...
							window['_PostActionContent'] && window['_PostActionContent']('在当前位置插入页码')
						}
            if (e.type !== 'click')
                this.toolbar.btnEditHeader.menu.hide();
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onBtnBlankPageClick: function(btn) {
            if (this.api){
							...
							this.api._doRecordCollaboration('新增空白页');
						}
        ...
        onListStyleSelect: function(combo, record) {
            ...
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置标题：${record.attributes.tip}`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
...
//        },
        ...
        onSelectHighlightColor: function(picker, color) {
          this._setMarkerColor(color, 'menu');
					this.api._doRecordCollaboration(`设置格式：颜色高亮：#${color}`)
        },
        ...
        onSelectFontColor: function(btn, color) {
            ...
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置格式：字体颜色：#${(typeof color === 'string')? color : color.color}`)
						}
        ...
        onBtnHighlightColor: function(btn) {
            if (btn.pressed) {
                ...
                this.api.SetMarkerFormat(false);
						...
						this.api._doRecordCollaboration(`设置格式：颜色高亮：#${btn.color}`)
        },
                        ...
                        if (me.api) {
                            ...
                            if (e.type !== 'click')
                                me.toolbar.btnInsertEquation.menu.hide();
														me.api._doRecordCollaboration('插入LaTeX公式')
