                    ...
                    if (this.btnExternal.isActive()) {//WebLink
                        if (this.inputUrl.checkValidate() !== true)  {
                            ...
                            return;
												...
												this.api._doRecordCollaboration(`将'${this.inputDisplay.getValue()}'添加链接:${this.inputUrl.getValue()}`)
                    } else {
                        var rec = this.internalList.getSelectedRec();
                        if (!rec || rec.get('level')==0 && rec.get('index')>0)
                            return;
												this.api._doRecordCollaboration(`将'${this.inputDisplay.getValue()}'添加链接:${rec.attributes.name}`)
                    }
