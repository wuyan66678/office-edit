    ...
    function CFontFilesCache()
    {
        ...
        this.LoadFontFile = function(stream_index, name, faceindex, fontManager)
        {
            if (!fontManager._engine)
							AscFonts.engine_Create && AscFonts.engine_Create(fontManager);

            // correct native stream
            if (AscFonts.CreateNativeStreamByIndex)
                AscFonts.CreateNativeStreamByIndex(stream_index);

            if (!(AscFonts.g_fonts_streams&&AscFonts.g_fonts_streams[stream_index]))
                return null;

            return fontManager._engine.openFont(AscFonts.g_fonts_streams && AscFonts.g_fonts_streams[stream_index], faceindex);
        };
