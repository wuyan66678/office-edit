...
(function(window, undefined)
{
	...
	CTextInput.prototype =
	{
		...
		init : function(target_id, parent_id)
		{
			...
			{
			  oThis.IsDisableKeyPress = false;
				oThis._needRecord = oThis.Api.isInShape && oThis.Api.isInShape() || false;
				if(oThis.Api && oThis.Api.WordControl && oThis.Api.WordControl.m_oLogicDocument) {
					let curLine = oThis.Api.WordControl.m_oLogicDocument.CurPos && oThis.Api.WordControl.m_oLogicDocument.CurPos.ContentPos
					oThis.curPosInfo = {
						curPage: oThis.Api.WordControl.m_oLogicDocument.CurPage,
						curLine: curLine,
						Type: oThis.Api.WordControl.m_oLogicDocument.CurPos && oThis.Api.WordControl.m_oLogicDocument.CurPos.Type
					}
				}
		...
		clear : function(isFromFocus)
		{
			...
			if(['xlsx', 'xls', 'xlsm'].includes(window['_FILE_TYPE'] && window['_FILE_TYPE'].toLowerCase() || [])) {
				if(text && text != ' ' && this._needRecord) {
					// 添加协作记录
					this.Api.onCInput && this.Api.onCInput(text);
				}
			} else {
				if(text && text != ' ') {
					this.Api.onCInput && this.Api.onCInput(text, this.curPosInfo);
				}
			}
			if (!this.TextArea_Not_ContentEditableDiv)
			{
