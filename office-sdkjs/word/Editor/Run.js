...
ParaRun.prototype.Remove = function(Direction, bOnAddText)
{
    ...
    var Selection = this.State.Selection;
		var oLogicDocument = this.GetLogicDocument();
		var oApi = oLogicDocument && oLogicDocument.GetApi() || null;
    ...
    else
    {
        if (true === Selection.Use)
        {
            ...
            {
                this.Remove_FromContent(StartPos, EndPos - StartPos, true, true);
								// 添加协作记录
								if(Direction == '-1') {
									// 当前编辑位置
									let actPos;
									if(this.Paragraph && this.Paragraph.LogicDocument) {
										actPos = {
											CurPage: this.Paragraph.LogicDocument.CurPage
										}
									}
									this.debounce(() => {
										if(this.deleteContent.length > 0) {
											let str = ''
											for(let i=this.deleteContent.length-1; i>=0; i--) {
												str += String.fromCharCode(this.deleteContent[i].Value)
											}
											oApi && oApi._doRecordCollaboration(`删除内容：${str}`, actPos)
											this.deleteContent = []
										}
									}, 500)
								}else {
        ...
        else
        {
            ...
            if (Direction < 0)
            {
                ...
                this.RemoveFromContent(CurPos - 1, 1, true, true);
                this.State.ContentPos = CurPos - 1;
								// 添加协作记录
								if(Direction == '-1') {
									// 当前编辑位置
...
ParaRun.prototype.Remove_FromContent = function(Pos, Count, UpdatePosition, recordContent)
{
	if (this.GetTextForm() && this.GetTextForm().IsComb())
		this.RecalcInfo.Measure = true;
	...
	if (History.CanAddChanges())
	{
		...
		if(recordContent) {
			this.deleteContent = this.deleteContent.concat(DeletedItems.slice().reverse())
		}
...
ParaRun.prototype.RemoveFromContent = function(nPos, nCount, isUpdatePositions, recordContent)
{
	return this.Remove_FromContent(nPos, nCount, isUpdatePositions, recordContent);
};
...
// 防抖
ParaRun.prototype.debounce = function (func, delay) {
	const context = this;
