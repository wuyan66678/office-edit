...
define([
    ...
    SSE.Views.DigitalFilterDialog = Common.UI.Window.extend(_.extend({
        ...
        save: function () {
            if (this.api && this.properties && this.rbOr && this.rbAnd &&
                ...
                this.api.asc_applyAutoFilter(this.needFilterShare, this.properties);
            }
        },
    ...
    SSE.Views.AutoFilterDialog = Common.UI.Window.extend(_.extend({
        ...
        initialize: function (options) {
                ...
                '<div class="box" style="height: 100%; display: flex; justify-content: space-between;">',
                    '<div class="content-panel" style="width: 100%; border-right: 1px solid #cbcbcb; display: flex; flex-direction: column; justify-content: space-between;">',
                        ...
                        '<div class="filter-share-container">',
                            '<div class="filter-share-txt">',
                                t.txtFilterShare,
                                '<div id="id-filter-share-tip" class="filter-share-tip">',
                                    "?",
                                '</div>',
                            '</div>',
                            '<div id="id-filter-share-switcher"></div>',
                        '</div>',
                        '<div class="footer center">',
            ...
            this.curSize = null;
            //改造--新增筛选是否分享switcher
            this.needFilterShare = options.needFilterShare;
        ...
        render: function () {
            ...
            this.switcherFilterShare = new Common.UI.Switcher({
                el:this.$window.find('#id-filter-share-switcher'),
                value: this.needFilterShare
            })
            this.switcherFilterShare.setDisabled(this.isView)
            if (this.switcherFilterShare) {
                this.switcherFilterShare.on('change', _.bind(this.onFilterShareChange, this));
                // _updateHint(this.switcherFilterShare, this.txtFilterShare + Common.Utils.String.platformKey('Ctrl+V'));
            }
            this.btnFilterShareTip = new Common.UI.Button({
                el:this.$window.find('#id-filter-share-tip')
            })
            if(this.btnFilterShareTip) {
                _updateHint(this.btnFilterShareTip, this.txtFilterShareTip);
            }
        ...
        onFilterShareChange: function (field, newValue) {
            this.needFilterShare = newValue
            this.fireEvent('needFilterShare', this, newValue);
        },
        ...
        onNumDynamicFilterItemClick: function(item) {
            ...
            this.api.asc_applyAutoFilter(this.needFilterShare, this.configTo);
            ...
            if (this.api && this.configTo && this.cells && this.filterExcludeCells) {
                ...
                if (isValid) {
                    ...
                    this.api.asc_applyAutoFilter(this.needFilterShare, this.configTo);
								...
								const cont = this.api.formatSelectedCells('的数据', '筛选')
								Common.Gateway.postCooperationHistory(cont)
            }
        },
