        ...
        doLayout: function() {
            ...
            this.panels.forEach(function(panel){
                if ( !panel.stretch ) {
                    ...
                    if ( style ) {
                         // 标题为绝对定位，不加入高度计算
                        height += (panel.rely!==true ? panel.el.is('#app-title')? 0 : panel.height : this.getElementHeight(panel.el));
                    }
            ...
            this.panels.forEach(function(panel){
                if (panel.el.is(':visible')) {
                    ...
                    panel.el.css(style);
                    // 标题为绝对定位，不加入高度计算
                    height += panel.el.is('#app-title')? 0 : style.height || this.getElementHeight(panel.el);
                }
            }, this);
        },
