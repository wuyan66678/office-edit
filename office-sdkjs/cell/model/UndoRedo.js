...
//для сохранения в историю и пересылки изменений
	...
	function UndoRedoData_SheetAdd(insertBefore, name, sheetidfrom, sheetid, tableNames, opt_sheet) {
		...
		//Эти поля заполняются после Undo/Redo 这些字段是在
		this.sheet = null;
	...
	//改造-- 新增表内容
	UndoRedoData_SheetAdd.prototype.setSheetLFixed = function (sheet) {
		this.sheet = sheet
	};
	...
	//для применения изменений 来应用变化
	...
	UndoRedoWorkbook.prototype.UndoRedo = function (Type, Data, nSheetId, bUndo, opt_wb) {
		...
		if (AscCH.historyitem_Workbook_SheetAdd == Type) {
			...
			if (bUndo) {
				...
				//保留已删除的工作表，否则可能会出现错误，如果某些对象记住了工作表的链接(例如:
				//Добавляем лист  -> Добавляем ссылку -> undo -> undo -> redo -> redo 添加列表->添加链接-> undo -> undo -> redo -> redo
				...
				if(Data.opt_sheet) {
					...
					//сюда заходим только если до этого было сделано Undo 只有在之前做过撤消之后才会进入这里
	...
	UndoRedoWoorksheet.prototype.UndoRedo = function (Type, Data, nSheetId, bUndo, opt_wb) {
		...
		//改造-- 筛选类型的undo根据新造类型来处理(只undo时)
		else if (AscCH.historyitem_Worksheet_RowLast == Type) {
			from = Data.from;
			to = Data.to;
			nRow = Data.bRow;
