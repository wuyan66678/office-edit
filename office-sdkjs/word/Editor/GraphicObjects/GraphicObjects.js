    ...
    remove: function(Count, bOnlyText, bRemoveOnlySelection, bOnTextAdd, isWord)
    {
        ...
        else if(this.selectedObjects.length > 0)
        {
					this.selectedObjects.forEach(item => {
						...
						let actCont = '';
						if(item.constructor === AscFormat.CImageShape) {
							actCont = '删除图片'
						} else if(item.constructor === AscFormat.CChartSpace) {
							actCont = '删除图表'
						} else if(item.constructor === AscFormat.CShape) {
							actCont = '删除形状'
						}
						if(actCont) {
							setTimeout(function () {
								oApi._doRecordCollaboration && oApi._doRecordCollaboration(actCont)
							} , 50)
						}
					})
            if(this.selection.groupSelection)
