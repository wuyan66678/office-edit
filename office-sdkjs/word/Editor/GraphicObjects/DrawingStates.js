    ...
    onMouseUp: function(e, x, y, pageIndex)
    {
        ...
        else
        {
            ...
            else
            {
				...
				if(aTracks[0] && this.bSamePos !== true) {
					let actionCont = '';
					if(aTracks[0] instanceof AscFormat.MoveShapeImageTrack) {
						actionCont = '移动了对象的位置'
					} else if(aTracks[0] instanceof AscFormat.ResizeTrackShapeImage) {
						actionCont = '修改了对象的大小'
					} else if (aTracks[0] instanceof AscFormat.RotateTrackShapeImage) {
						actionCont = '修改了对象的旋转角度'
					}
					if(actionCont) {
						try {
							this.drawingObjects.drawingDocument.m_oWordControl.m_oApi._doRecordCollaboration(actionCont)
						} catch(e){}
					}
				}
        this.drawingObjects.changeCurrentState(new NullState(this.drawingObjects));
