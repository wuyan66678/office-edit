    ...
    DE.Views.ParagraphSettings = Backbone.View.extend(_.extend({
        ...
        openAdvancedSettings: function(e) {
            ...
            if (me.api && !this._locked){
                                ...
                                handler: function(result, value) {
                                    if (result == 'ok') {
                                        if (me.api) {
																					...
																					try {
																						me.parseParagraphAct(value.paragraphProps)
																					} catch(e) {}
                                        }
                                    }
				...
				parseParagraphAct: function (props) {
					let me = this;
					let actPagingContent = [];
					if(props.PageBreakBefore) {
						actPagingContent.push('段前分页')
					}
					if(props.WidowControl) {
						actPagingContent.push('单独控制')
					}
					if(props.SuppressLineNumbers) {
						actPagingContent.push('压制行号')
					}
					if(props.KeepLines) {
						actPagingContent.push('保持同一行')
					}
					if(props.KeepNext) {
						actPagingContent.push('与下一个保持一致')
					}
					if(actPagingContent.length > 0) {
						me.api._doRecordCollaboration(`设置段落分页：${actContent.join('、')}`)
					}
					let fontActContent = []
					if(props.AllCaps) {
						fontActContent.push('全部大写字母');
					}
					if(props.SmallCaps) {
						fontActContent.push('小型大写字母');
					}
					if(props.Strikeout) {
						fontActContent.push('删除线');
					}
					if(props.Subscript) {
						fontActContent.push('下标');
					}
					if(props.Superscript) {
						fontActContent.push('上标');
					}
					if(props.DStrikeout) {
						fontActContent.push('双删除线');
					}
					if(fontActContent.length > 0) {
						me.api._doRecordCollaboration(`设置段落字体：${fontActContent.join('、')}`)
					}
					if(props.Shd && props.Shd.Fill) {
						let color = props.Shd.Fill.get_hex().substring(0, 6)
						me.api._doRecordCollaboration(`设置段落背景色：#${color}`)
					}
				},
