            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!this.drawingObjects.checkSelectedObjectsProtection())
                    {
						...
						if(tracks.length > 0) {
							let actionCont = '';
							if(tracks[0] instanceof AscFormat.MoveShapeImageTrack) {
								actionCont = '移动了对象的位置'
							} else if(tracks[0] instanceof AscFormat.ResizeTrackShapeImage) {
								actionCont = '修改了对象的大小'
							} else if (tracks[0] instanceof AscFormat.RotateTrackShapeImage) {
								actionCont = '修改了对象的旋转角度'
							}
							if(actionCont) {
								var oApi = oThis.drawingObjects.getEditorApi();
								if(oApi && oApi._doRecordCollaboration) {
									oApi._doRecordCollaboration(actionCont);
								} else {
