...
function (window, undefined) {
	...
	window['AscCH'].historyitem_Workbook_SheetAdd = 1;//表新增
	window['AscCH'].historyitem_Workbook_SheetRemove = 2;//表删除
	...
	window['AscCH'].historyitem_Worksheet_RemoveCell = 1;//删除格
	window['AscCH'].historyitem_Worksheet_RemoveRows = 2;//删除行
	window['AscCH'].historyitem_Worksheet_RemoveCols = 3;//删除列
	window['AscCH'].historyitem_Worksheet_AddRows = 4;//新增行
	window['AscCH'].historyitem_Worksheet_AddCols = 5;//新增列
	...
	//改造-- 用于记录上次筛选状态的类型
	window['AscCH'].historyitem_Worksheet_RowLast = 101;
	window['AscCH'].historyitem_Worksheet_RowFilter = 102;//改造-- 用于记录上次筛选信息的类型
// Frozen cell
...
{	
	//改造--撤回参数
	this.IndexLFixed = -1;
	this.LastUndoSaveLFixed = false;
	this.UndoRedoPointLFixed = [];//撤回重做记录节点供发送更改
	this.workbook = null;
    this.Index    = -1;
    this.Points   = [];
    this.TurnOffHistory = 0;
    this.Transaction = 0;
		//если true все добавленный изменения не пойдут в совместное редактирование.
		//如果所有添加的更改都不进行联合编辑。
	...
	this.SavedIndex = null;			//最后一次保存发生的回收点号 Номер точки отката, на которой произошло последнее сохранение
...
{
	const bIsCollaborative = this.workbook.oApi.collaborativeEditing.getCollaborativeEditing()
	if(bIsCollaborative) {
			this.IndexLFixed = -1
...
CHistory.prototype.ClearLFixed = function()
{
	this.IndexLFixed = -1
	// this.Index         = -1;
	// this.Points.length = 0;
...
//改造--转存到自己的参数
CHistory.prototype.SaveChangeLFixed = function() {
	this.IndexLFixed = this.Index
}
//改造--转存到自己的参数
CHistory.prototype.SetLastUndoSaveLFixed = function() {
	if(this.LastUndoSaveLFixed && this.IndexLFixed === -2){
		this.LastUndoSaveLFixed = false;
		this.IndexLFixed = -1
	}
}
...
{
  // Проверяем можно ли сделать Undo 看看undo是否能做到
  if (true !== this.Can_Undo()) {
	...
	var t = this;
	var doUndo = function () {
		Point.isUndoLFixed = true
		t.UndoRedoPointLFixed.push(Point)
		if(Point.isFilter && Point.Items[0]) {
			Point.Items.forEach(item => {
				item.LocalChange = true
			})
			t.workbook.handlers.trigger("asc_onSetUndoFilterFlag", true);
			t.workbook.needShareFilterLFixed = false
		}
	...
	// Откатываем все действия в обратном порядке (относительно их выполенения) 向后滚动所有动作
	var Point = null;
...
CHistory.prototype.UndoRedoPrepare = function (oRedoObjectParam, bUndo) {
	...
	// 在需要重新计票的情况下关闭附件，在需要重新计票的情况下将其添加到重新计票列表中。
...
CHistory.prototype.UndoRedoEnd = function (Point, oRedoObjectParam, bUndo) {
	...
	if (null != Point) {
		...
		//синхронизация index и id worksheet 同步index和id worksheet
		...
		for (i in Point.UpdateRigions) {
			...
			//最后一个参数是在直接操作时添加/删除行/列时传递resetCache。
...
CHistory.prototype.Redo = function()
{
	...
	Point.isUndoLFixed = false
	Point.isRedoLFixed = true
	if(Point.isValueChangeLFixed) this.UndoRedoPointLFixed.push(Point)
	//改造-- undo/redo全视为本地操作
	if(Point.isFilter && Point.Items[0]) {
		Point.Items.forEach(item => {
			item.LocalChange = true
		})
		this.workbook.handlers.trigger("asc_onSetUndoFilterFlag", true);
		this.workbook.needShareFilterLFixed = false
	}
	
	this.RedoExecute(Point, oRedoObjectParam);
	this.SaveChangeLFixed()
...
CHistory.prototype.Create_NewPoint = function(isPaste)
{
    ...
    // Создаем новую точку 创造一个新的点
    this.Points[++this.Index] = {
		Items : Items, // Массив изменений, начиная с текущего момента 从现在开始的一系列变化
...
// Регистрируем новое изменение: 我们正在记录一个新的变化:
// Class - объект, в котором оно произошло 它发生的对象
// Data  - сами изменения
CHistory.prototype.Add = function(Class, Type, sheetid, range, Data, LocalChange)
{
	...
	curPoint.hasBeenSaved = false;
	//改造-- 给过滤方法打标记并删除其他过滤历史
	if(Class && Class.nType === 8 && Type === 5 ) {
		// var index = this.Points.findIndex(x=> x.isFilter)
		// if(index !== -1 && index != this.Index){
		// 	this.Points.splice(index,1)
		// }
		curPoint.isFilter = true

	}else if([16,1,2,3,4,5].find(x=> x === Type)) { 
		//改造-- 若为重做与撤销需另起上传节点的类型（如今历史节点未清空，撤销与重做时上传最后一步需要新起临时节点。
		//16.值类型修改，1.删除格/新增表 2.删除行/表删除，3.删除列，4.新增行，5.新增列
		curPoint.isValueChangeLFixed = true
	}
	if(Item.LocalChange){
		curPoint.isLocalChangeLFixed = true
	}
	if(!this.workbook.oApi.collaborativeEditing.m_bFast){
		curPoint.isStrictLFixed = true //严格模式
	}
...
CHistory.prototype.Is_On = function()
{
	...
	CHistory.prototype.GetDeleteIndex = function() {
		...
		for (var i = 0; i < DeletePointIndex; ++i) {
			...
			for (var j = 0; j < point.Items.length; ++j) {
				if (!point.Items[j].LocalChange) {//LocalChange изменения не пойдут в совместное редактирование.改变不会变成联合编辑。
					DeleteIndex += 1;
				}
			}
		}
	...
	CHistory.prototype.Have_Changes = function(IsNotUserSave) {
		const bIsCollaborative = this.workbook && this.workbook.oApi.collaborativeEditing.getCollaborativeEditing()
		if(bIsCollaborative){
			var checkIndex = (this.Is_UserSaveMode() && !IsNotUserSave) ? this.UserSavedIndex : this.SavedIndex;
			if(-2 === this.IndexLFixed && this.LastUndoSaveLFixed) {
				return true
			}
			if (-1 === this.IndexLFixed && null === checkIndex && false === this.ForceSave) {
				return false;
			}

			return (this.IndexLFixed != checkIndex || true === this.ForceSave);
		} else {
			var checkIndex = (this.Is_UserSaveMode() && !IsNotUserSave) ? this.UserSavedIndex : this.SavedIndex;
...
/** @returns {boolean} */
CHistory.prototype.Have_ChangesLFixed = function(IsNotUserSave) {
	var checkIndex = (this.Is_UserSaveMode() && !IsNotUserSave) ? this.UserSavedIndex : this.SavedIndex;
	if(-2 === this.IndexLFixed && this.LastUndoSaveLFixed) {
		return true
	}
...
CHistory.prototype.GetSerializeArray = function()
{
	...
	var i = 0;
	var filterIndex = []
	//改造-- 拼接另外记录的undo，redo节点到最后，并清空当前记录
	var curPoints = []
	console.log('this.Points', this.Points)
	curPoints.push(...this.Points)
	if(this.UndoRedoPointLFixed.length > 0){
		curPoints = curPoints.concat(this.UndoRedoPointLFixed)
		this.UndoRedoPointLFixed = []
	}
	if (null != this.SavedIndex)
		i = this.SavedIndex + 1;
	for(; i < curPoints.length; ++i)
	{
		var point = curPoints[i];
		var aPointChanges = [];
		aPointChanges.hasBeenSaved = point.hasBeenSaved
		for(var j = 0, length2 = point.Items.length; j < length2; ++j)
		{
			...
			aPointChanges.isUndoLFixed = point.isUndoLFixed
			aPointChanges.isRedoLFixed = point.isRedoLFixed
			aPointChanges.isLocalChangeLFixed = point.isLocalChangeLFixed
			aPointChanges.isFilter = point.isFilter
			aPointChanges.isStrictLFixed = point.isStrictLFixed
			aPointChanges.isPaste = point.isPaste
		}
		aRes.push(aPointChanges);
		if(point.isFilter) filterIndex.push(aRes.length-1)
	}
	//改造-- 只保留最后的更改,与未上传的本地筛选

	if( this.workbook.oApi.collaborativeEditing.getCollaborativeEditing()){
		for(let item of this.Points){
			if(item.isLocalChangeLFixed && item.isFilter && this.workbook.needShareFilterLFixed){
				item.isLocalChangeLFixed = false
			}
			if(item.isStrictLFixed) {
				item.isStrictLFixed = false
			}
		}
		for(var i = aRes.length-1; i >= 0; i--) {
			if(aRes[i].isStrictLFixed) {
				aRes[i].isStrictLFixed = false
			}else if(aRes[i].isLocalChangeLFixed && aRes[i].isFilter && this.workbook.needShareFilterLFixed){//本地修改且需要同步
				aRes[i].isLocalChangeLFixed = false
			}else {
				if(i !== aRes.length-1){
					aRes.splice(i,1)
				}
	...
	/* setTimeout(() => {
		// bug修复 增加一个状态hasBeenSaved，解决协同编辑时，删除行或新增行会重复执行问题，场景：新增行，双击单元格，然后单击其他单元格，会重复执行新增行操作
		this.Points.forEach(pitem => {
			//增加一个状态hasBeenSaved,确保此次操作只执行一次
			pitem.hasBeenSaved = true
			pitem.isUndoLFixed = false
			pitem.isRedoLFixed = false
		})
	}, 0) */
	return aRes;
};
CHistory.prototype.setPointsState = function () {
	this.Points.forEach(pitem => {
		//增加一个状态hasBeenSaved,确保此次操作只执行一次
		pitem.hasBeenSaved = true
		pitem.isUndoLFixed = false
	 ...
	 * 从保存在服务器上的历史记录中删除更改。这发生在第二个用户连接时
	 */
	CHistory.prototype.RemovePointsByDeleteIndex = function()
	{
		const bIsCollaborative = this.workbook.oApi.collaborativeEditing.getCollaborativeEditing()
		if(bIsCollaborative) //协同编辑也不删除历史
		{
			this.SavedIndex = null;
			return
		}
