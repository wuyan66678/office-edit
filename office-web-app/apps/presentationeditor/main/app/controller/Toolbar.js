...
//            this.api.asc_selectSearchingResults(false);
        ...
        onAddSlide: function(type) {
            if ( this.api) {
                this.api.AddSlide(type);
								window['_PostActionContent']('插入幻灯片');
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onBold: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置文本格式：字体：加粗')
							} else {
								this.api._doRecordCollaboration('设置文本格式：字体：取消加粗')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onItalic: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置文本格式：字体：斜体')
							} else {
								this.api._doRecordCollaboration('设置文本格式：字体：取消斜体')
							}
						}
        ...
        onUnderline: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置文本格式：字体：下划线')
							} else {
								this.api._doRecordCollaboration('设置文本格式：字体：取消下划线')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onStrikeout: function(btn, e) {
            ...
            if (this.api) {
							...
							if (btn.pressed) {
								this.api._doRecordCollaboration('设置文本格式：字体：删除线')
							} else {
								this.api._doRecordCollaboration('设置文本格式：字体：取消删除线')
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onSuperscript: function(btn, e) {
            if (!this.toolbar.btnSubscript.pressed) {
                ...
                if (this.api) {
									...
									if (btn.pressed) {
										this.api._doRecordCollaboration('设置文本格式：字体：上标')
									} else {
										this.api._doRecordCollaboration('设置文本格式：字体：取消上标')
									}
								}
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onMenuHorizontalAlignSelect: function(menu, item) {
            ...
            if (this.api && item.checked) {
							...
							switch(item.value) {
								case 1:
									actCont = '设置文本格式：对齐：左对齐';
									break;
								case 2:
									actCont = '设置文本格式：对齐：居中对齐';
									break;
								case 0:
									actCont = '设置文本格式：对齐：右对齐';
									break;
							}
							if(actCont) {
								this.api._doRecordCollaboration(actCont)
							}
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onLineSpaceToggle: function(menu, item, state, e) {
            if (!!state) {
                ...
                if (this.api) {
									...
									this.api._doRecordCollaboration(`设置行间距：${item.value}`)
								}
                Common.NotificationCenter.trigger('edit:complete', this.toolbar);
            ...
            if (this.api) {
                if (item.value == 'advanced') {
                    ...
                    this.api.ShapeApply(props);
										this.api._doRecordCollaboration(`设置列格式：${item.caption}`)
                }
            }
        ...
        onListThemeSelect: function(combo, record) {
            ...
            if (this.api && record) {
							...
							this.api._doRecordCollaboration(`设置主题：${record.attributes.tip}`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onSelectFontColor: function(btn, color) {
            ...
            if (this.api) {
							...
							this.api._doRecordCollaboration(`设置文本格式：字体颜色：#${(typeof color === 'string')? color : color.color}`)
						}
        ...
        onBtnHighlightColor: function(btn) {
            if (btn.pressed) {
                ...
                this.api.SetMarkerFormat(false);
						...
						this.api._doRecordCollaboration(`设置文本格式：颜色高亮：#${btn.color}`)
        },

        onSelectHighlightColor: function(picker, color) {
            this._setMarkerColor(color, 'menu');
						this.api._doRecordCollaboration(`设置文本格式：颜色高亮：#${color}`)
        },
