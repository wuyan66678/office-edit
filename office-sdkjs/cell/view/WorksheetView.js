 ...
 */
	...
	WorksheetView.prototype.changeColumnWidth = function (col, x2, mouseX) {
		...
		var onChangeWidthCallback = function (isSuccess) {
			...
			t.changeWorksheet("update", {reinitRanges: true}, true); //改造-- 新增第三的参数：是否为行列宽度修改
	...
	WorksheetView.prototype.changeRowHeight = function (row, y2, mouseY) {
		...
		var onChangeHeightCallback = function (isSuccess) {
			...
			t.changeWorksheet("update", {reinitRanges: true}, true);
...
//  NOTE: если проекция строчки на Y больше высоты ячейки подставлять # и рисовать все по центру
    ...
    WorksheetView.prototype.setSelectionInfo = function (prop, val, onlyActive, isMineComments) {
        ...
        var onSelectionCallback = function (isSuccess) {
			...
			//改造-- TODO:paste走
				var onSelectionCallbackLFixed = function (isSuccess) {
					if (false === isSuccess) {
							return;
					}
					var hasUpdates = false;
					...
					t.model.workbook.handlers.trigger("cleanCutData", true, true);
						...
						//в случае, если вставляем из глобального буфера, транзакцию закрываем внутри функции _loadDataBeforePaste на callbacks от загрузки шрифтов и картинок
						// /如果我们从全球缓冲区插入，交易将在函数_loadadabeforepaste中关闭。
					if (prop !== "paste") {
						History.EndTransaction();
						if(expansionTableRange) {
							t.applyTableAutoExpansion(expansionTableRange);
		...
		if ("paste" === prop) {
			if (val.onlyImages) {
				...
				if (val.fromBinary) {
					//при вставке извне не работает эта обработка в мс 外部插入不能在mc中工作
					//не клонирую, поскольку нигде меняться не будет 我不会克隆它，因为它不会改变任何地方。
					val.originalSelectBeforePaste = this.model.selectionRange ? this.model.selectionRange.clone() : null;
					//改造--走改造的选取格不需要先给过滤选取分段（原代码因筛选而分段截取） lxt
				...
				var newRange = val.fromBinary ? this.pasteFromBinaryLFixed(val.data, true) : this._pasteFromHTML(val.data, true);
				console.log('newRange', newRange)
				//改造--改造后的为二维数组
				checkPasteRange = newRange && newRange.length ? newRange : [newRange];
				var tempRange = checkPasteRange[0]
				checkRange = [checkPasteRange[0]];
				if(Array.isArray(tempRange)){
					checkRange = [tempRange[0]]
				}
				...
				if (!newRange) {
					...
					var api = window["Asc"]["editor"];
					console.log('newRange', newRange)
					if(newRange[0] && newRange[0][0]) {
						api.postActionContent('', '粘贴', newRange[0][0])
					} else {
						api.postActionContent('', '粘贴')
					}
				}
				if(val.fromBinary) {
					//改造--改造后的循环二维数组
					for (var j = 0; j < checkPasteRange.length; j++) {
						var _checkRanges = checkPasteRange[j];
						for(var k = 0; k < _checkRanges.length; k++){
							var _checkRange = _checkRanges[k]
	...
	WorksheetView.prototype._loadDataBeforePaste = function (isLargeRange, val, bIsUpdate, canChangeColWidth, pasteToRange) {
		...
		var callback = function () {
			for (var j = 0; j < pasteToRange.length; j++) {
				if(Array.isArray(pasteToRange[j])) {
					pasteToRange[j].forEach(item => {
						_doPaste(item);
					})
				}else {
	...
	//改造--返回筛选后可见选中区域 lxt
	WorksheetView.prototype.pasteFromBinaryLFixed = function (val, isCheckSelection, tablesMap, pasteToRange) {
		// TODO: 筛选后返回的粘贴范围需改造
		var pasteRange = AscCommonExcel.g_clipboardExcel.pasteProcessor.activeRange;
		if (typeof pasteRange === "string") {
			AscCommonExcel.executeInR1C1Mode(false, function () {
		...
		var res;
		//isCheckSelection必为true
		if (isCheckSelection) {
			for (i = 0; i < selectRanges.length; i++) {
				var _selection = this._pasteFromBinaryLFixed(val, isCheckSelection, tablesMap, selectRanges[i]);
				if (!res) {
					res = [];
				}
				res.push(_selection);
	...
	//改造--按粘贴数量选区改造（粘贴可见区域 lxt isCheckSelection为true才进入该方法
	WorksheetView.prototype._pasteFromBinaryLFixed = function (val, isCheckSelection, tablesMap, pasteInRange) {
		var t = this;
		...
		//粘贴起始列+粘贴跨列数计算最大使用行列数（列数需改造

		//改造--
		//#region 获取row,range的列表
		var getRangeList = function() {
			var needCount = countPasteRow;
			var getCount = 0
			var r1 = arn.r1;
			var r2 = countPasteRow + arn.r1 -1;
			var tempRanges = []
			var rMax
			while(getCount < needCount) {
				var tempRange = getVisualRange(r1, r2)
				if(tempRange.r1 !== null) {
					if(!tempRange.r2) tempRange.r2 = tempRange.r1
					if (tempRange.r1 > gc_nMaxRow0) {
						tempRange.r1 = gc_nMaxRow0;
					}
					if (tempRange.r2 > gc_nMaxRow0) {
						tempRange.r2 = gc_nMaxRow0;
					}
					tempRange.index = getCount
					tempRanges.push(tempRange)
					getCount += (tempRange.r2-tempRange.r1 + 1)
					r1 = tempRange.r2
					rMax = tempRange.r2
					if (rMax > gc_nMaxRow0) {
						rMax = gc_nMaxRow0;
				...
				r1++
				r2 = r1 + countPasteRow - getCount - 1
			}
			return {rMax,tempRanges}

		}
		//获取可见行
		var getVisualRange = function(startRow, endRow) {
			var tempRange = {
				r1:null,
				r2:null
			}
			for (var j = startRow; j <= endRow; j++) {
				...
				if(isHidden) break
				if(tempRange.r1 === null) tempRange.r1 = j
				else tempRange.r2 = j
			}
			return tempRange
		}
		//#endregion
		var res = getRangeList()
		var rMax = res.rMax
		var visualRowRanges = res.tempRanges
		...
		if (!isMergedFirstCell) {
			if (arn.c1 === arn.c2 && arn.r2 > arn.r1 && countPasteCol > 1 && countPasteRow === 1) {
				//если всталяем одну строку в одну колонку 如果我们把一行放在一个列中
				...
				//если всталяем одну колонку в одну строку 如果我们把一个列放在一行里
        ...
        //если вставляем в мерженную ячейку, диапазон которой больше или меньше, но не равен выделенной области
				//如果你把它插入一个死亡细胞，它的范围是或多或少，但不等于指定的区域
        if (isMergedFirstCell && isMergedFirstCell.isEqual(arn) && cMax - arn.c1 === (firstValuesCol + 1) &&
          rMax - arn.r1 === (firstValuesRow + 1) && !newPasteRange.isEqual(activeCellsPasteFragment)) {
            ...
            //если область кратная куску вставки 如果一个区域是一个多重插入区
            ...
            //если кратны, то обрабатываем 简而言之，我们正在处理。
            if (widthArea % widthPasteFr === 0 && heightArea % heightPasteFr === 0) {
				//Для случая, когда выделен весь диапазон, запрещаю множественную вставку 对于所有的范围，禁止复位。
			...
			} else if (firstCell.hasMerged() !== null)//в противном случае ошибка 否则，错误
			{
				if (isCheckSelection) {
					//return arn;
				} else {
			...
			//проверка на наличие части объединённой ячейки в области куда осуществляем вставку 检查我们插入的区域内是否有部分联合细胞。
			for (var rFirst = arn.r1; rFirst < rMax; ++rFirst) {
				for (var cFirst = arn.c1; cFirst < cMax; ++cFirst) {
					var range = t.model.getRange3(rFirst, cFirst, rFirst, cFirst);
					var merged = range.hasMerged();
					if (merged) {
						if (merged.r1 < arn.r1 || merged.r2 > rMax - 1 || merged.c1 < arn.c1 || merged.c2 > cMax - 1) {
							//ошибка в случае если вставка происходит в часть объедененной ячейки 嵌入到统一单元中的错误
							if (isCheckSelection) {
								return arn;
							} else {
								this.handlers.trigger("onErrorEvent", c_oAscError.ID.PastInMergeAreaError,
		...
		var cMax2 = cMax;
		//改造--预计改造为返回相应数量数组
		var getLockRange = function (temp) {
			var newArr = arn.clone(true);
			newArr.r1 = temp.r1
			newArr.r2 = temp.r2;
			newArr.c2 = cMax2 - 1;
		...
		var getLockRangesLFixed = function() {
			var resRanges = []
			for(let temp of visualRowRanges){
				var range = getLockRange(temp)
	...
	WorksheetView.prototype._pasteFromBinary = function (val, isCheckSelection, tablesMap, pasteInRange) {
		...
		var oldActiveCellsPasteFragmentParam = {
			r1:activeCellsPasteFragment.r1,
			r2:activeCellsPasteFragment.r2
		}
		if(pasteInRange.index >= 0){
			activeCellsPasteFragment.r1 += pasteInRange.index
			activeCellsPasteFragment.r2 = activeCellsPasteFragment.r1 + pasteInRange.r2-pasteInRange.r1
		}
		var specialPasteHelper = window['AscCommon'].g_specialPasteHelper;
		var specialPasteProps = specialPasteHelper.specialPasteProps;
		...
		//还原？
		activeCellsPasteFragment.r1 = oldActiveCellsPasteFragmentParam.r1
		activeCellsPasteFragment.r2 = oldActiveCellsPasteFragmentParam.r2
		return [trueActiveRange, arrFormula];
	};
	...
	WorksheetView.prototype.changeSelectOnMultiSelect = function () {
		//разбиваем селект в зависимости от наличия скрытых строк внутри фильтра 根据过滤器内部隐藏的字符串的存在，将sellex分解
		var t = this;
		var breakRange;
		if (this.model.AutoFilter && this.model.AutoFilter.isApplyAutoFilter()) {
			//отсеиваем все скрытые строки 清除所有隐藏的字符串。
			...
			//检查是否有一个活跃的f / t单元使用了a / f，并根据此列出隐藏在字符串中的列表。
		...
		var breakRangeByHiddenRows = function (_range, intersection) {
			//чтобы не усложнять логику прохожусь по всем строкам селекта 为了让逻辑更简单，我遵循了select的所有行。
			var tempRanges = [];
			var tempRange;
		...
		if (breakRange) {
			...
			for (var i = 0; i < sr.ranges.length; i++) {
				if (sr.ranges[i]) {
					...
					if (intersection) {
						//нужно пройтись по всем строкам пересечения и отсеять скрытые 我们需要检查所有的交叉线，排除隐藏的。
	...
	//改造-- 新增是否为修改行列宽度参数
	WorksheetView.prototype.changeWorksheet = function (prop, val, isCRWidthChange ) {
	...
	WorksheetView.prototype.addAutoFilter = function (styleName, addFormatTableOptionsObj) {
		...
		// Проверка глобального лока 检查全局锁
		//改造-- 使预览模式也可筛选
		var isView = this.workbook.Api.DocInfo.Mode === 'view'
		if (this.collaborativeEditing.getGlobalLock() || (!window["Asc"]["editor"].canEdit() && !isView)) {

			return;
		}

		if (!this.handlers.trigger("getLockDefNameManagerStatus") && !isView) {
			this.handlers.trigger("onErrorEvent", c_oAscError.ID.LockCreateDefName, c_oAscError.Level.NoCritical);
			return;
		}
		...
		var onChangeAutoFilterCallback = function (isSuccess) {
			...
			if (bIsChangeFilterToTable)//CHANGE FILTER TO TABLEPART
			{
				addFilterCallBack = function (isSuccess) {
					...
					!isView && History.Create_NewPoint();
					!isView && History.StartTransaction();
			...
			} else//ADD
			{
				addFilterCallBack = function (isSuccess) {
					...
					var slowOperationCallback = function() {
						...
						//必须在_onUpdateFormatTable之前强制重新计算
						//для грамотного отображения формул(#46170) 公式的正确显示
						//TODO пересчёт происходит два раза! в unlockRecal и EndTransaction. 重新计算两次!到unlockRecal和EndTransaction。
						t.model.workbook.dependencyFormulas.unlockRecal();
	...
	WorksheetView.prototype.changeAutoFilter = function (tableName, optionType, val, opt_callback) {
		// Проверка глобального лока
		//改造-- 预览模式下可筛选
		if(this.workbook.Api.DocInfo.Mode !== 'view') {
			if (this.collaborativeEditing.getGlobalLock() || !window["Asc"]["editor"].canEdit()) {
				if (opt_callback) {
					opt_callback(false);
				}
	...
	/**
	 * 
	 * @param {boolean} needShareFilter (改造--是否同步筛选项)
	 * @param {*} autoFilterObject 
	 * @returns 
	 */
	WorksheetView.prototype.applyAutoFilter = function (needShareFilter, autoFilterObject) {
		...
		var onChangeAutoFilterCallback = function (isSuccess) {
			...
			var applyFilterProps = t.model.autoFilters.applyAutoFilterLfixed(needShareFilter,autoFilterObject, ar);
			if (!applyFilterProps) {
				return false;
			}
		...
		if (!window['AscCommonExcel'].filteringMode) {
			...
			//即使在视图模式下。为了避免cofflicks，假设一个用户在分类
			//второй - фильтрует
			//另一个是过滤。
			this._isLockedAll(onChangeAutoFilterCallback);
		}
	...
	WorksheetView.prototype.clearFilter = function () {
		...
		//в особом режиме не лочим лист при фильтрации 在特殊模式下，过滤时不除草
		var nActive = t.model.getActiveNamedSheetViewId();
		var onChangeAutoFilterCallback = function (isSuccess) {
    ...
    WorksheetView.prototype._onUpdateFormatTable = function (range, lockDraw) {
			// ToDo сделать правильное обновление при скрытии/раскрытии строк/столбцов ToDo在隐藏/揭示行/列时进行正确的更新
			this._initCellsArea(AscCommonExcel.recalcType.full);
			this.cache.reset();
			this._cleanCellsTextMetricsCache();
			this._prepareCellTextMetricsCache();
