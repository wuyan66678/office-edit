...
(
	...
	function (window, undefined) {
		...
		AutoFilters.prototype = {
			...
			/**
			 * 改造筛选代码，可选是否同步
			 * @param {*} needShareFilter  是否需要同步筛选
			 * @param {*} autoFiltersObject 
			 * @param {*} ar 
			 * @param {*} tryConvertFilter 
			 * @returns 
			 */
			applyAutoFilterLfixed: function (needShareFilter, autoFiltersObject, ar, tryConvertFilter) {
				
				var worksheet = this.worksheet;
				//改造-- 20240131
				worksheet.workbook.needShareFilterLFixed = needShareFilter
				...
				History.Create_NewPoint();
				History.StartTransaction();
				var fixedOldLocalChange = History.LocalChange;
				//改造--开启本地模式，不同步筛选
				if(!needShareFilter) {
					History.LocalChange = true;
				}
				...
				this._addHistoryObj(oldFilter, AscCH.historyitem_AutoFilter_Apply,
				{activeCells: ar, autoFiltersObject: autoFiltersObject}, null, rangeOldFilter);
				History.LocalChange = fixedOldLocalChange;
				History.EndTransaction();
			...
			Undo: function (type, data) {
				...
				var undo_apply = function() {
					if (cloneData.Ref) {
						...
						if (!_doAdd)//добавляем фильтр 添加过滤器
						{
							if (cloneData.TableStyleInfo) {
			...
			_addHistoryObj: function (oldObj, type, redoObject, deleteFilterAfterDeleteColRow, activeHistoryRange,
				...
				History.Add(AscCommonExcel.g_oUndoRedoAutoFilters, type, ws.getId(), activeHistoryRange,
					oHistoryObject);
				if(type === 5 && this.lastOHistoryObject) { //上次筛选信息
					History.Add(AscCommonExcel.g_oUndoRedoAutoFilters, AscCH.historyitem_Worksheet_RowFilter, ws.getId(), activeHistoryRange,
					this.lastOHistoryObject)
					
				}
				this.lastOHistoryObject = oHistoryObject
			},
			...
			_reDrawCurrentFilter: function (fColumns, tableParts) {
				//TODO сделать открытие и закрытие строк 打开和关闭字符串
				//перерисовываем таблицу со стилем  用风格重做表
				if (tableParts) {
