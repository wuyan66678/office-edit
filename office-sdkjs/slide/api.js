...
(function(window, document)
{
	...
	asc_docs_api.prototype.CreateCSS = function()
	{
		...
		style0.innerHTML = ".block_elem { position:absolute;padding:0;margin:0;z-index:0 }.thmbnl_newpage{position:fixed;bottom:0;right:0;height:85px;line-height:85px;width:30px;text-align:center;font-size:20px;font-weight:bold;z-index:100;background:rgb(244, 244, 244);cursor:pointer;border-left:1px solid #d9d9d9;border-top: 1px solid #d9d9d9;display:none;}";
...
background-repeat: no-repeat;\
	...
	asc_docs_api.prototype.CreateComponents = function()
	{
		...
		var _innerHTML = "<div id=\"id_main_parent\" class=\"block_elem\" style=\"touch-action:none;-ms-touch-action: none;-moz-user-select:none;-khtml-user-select:none;user-select:none;overflow:hidden;border-left-width: 1px;border-left-color:" + AscCommon.GlobalSkin.BorderSplitterColor + "; border-left-style: solid;\" UNSELECTABLE=\"on\">\
			...
			_innerHTML = "<div id=\"id_thmbnl_newpage\" class=\"thmbnl_newpage\"> + </div>\
			<div id=\"id_panel_thumbnails\" class=\"block_elem\" style=\"touch-action:none;-webkit-touch-callout:none;background-color:" + AscCommon.GlobalSkin.BackgroundColorThumbnails + ";border-top:1px solid #d9d9d9;position:fixed;display:none;\">\
			<div id=\"id_panel_thumbnails_split\" class=\"block_elem\" style=\"pointer-events:none;background-color:" + AscCommon.GlobalSkin.BackgroundColorThumbnails + ";\"></div>\
										...
										<div id=\"id_vertical_scroll_thmbnl\" style=\"left:0;top:0;width:1px;overflow:hidden;position:absolute;\"></div>\
								</div>\
		...
		<div id=\"id_panel_left\" class=\"block_elem\">\
			...
			<canvas id=\"id_hor_ruler\" class=\"block_elem\"></canvas>\
										...
										<div id=\"id_main_view\" class=\"block_elem\" style=\"overflow:auto\">\
												<canvas id=\"id_viewer\" class=\"block_elem\" style=\"-ms-touch-action: none;-webkit-user-select: none;background-color:" + AscCommon.GlobalSkin.BackgroundColor + ";z-index:6\"></canvas>\
												<canvas id=\"id_viewer_overlay\" class=\"block_elem\" style=\"-ms-touch-action: none;-webkit-user-select: none;z-index:7\"></canvas>\
												<div id=\"id_target_cursor\" class=\"block_elem\" width=\"1\" height=\"1\" style=\"-ms-touch-action: none;-webkit-user-select: none;width:2px;height:13px;display:none;z-index:9;\"></div>\
										</div>\
		...
		if (true)
		{
			_innerHTML += "<div id=\"id_bottom_pannels_container\" class=\"block_elem\" style=\"-ms-touch-action: none;-moz-user-select:none;-khtml-user-select:none;user-select:none;overflow:hidden;background-color:" + AscCommon.GlobalSkin.BackgroundColorNotes + ";z-index:10;\">\
		...
		console.log('this.HtmlElement v5----', this.HtmlElement);
	};
	...
	asc_docs_api.prototype.asc_setSkin = function(theme)
	{
		...
		if (obj_id_main)
		{
			...
			document.getElementById("id_panel_thumbnails").style.backgroundColor = '#f6f7f8';
	...
	asc_docs_api.prototype.asc_PasteData = function(_format, data1, data2, text_data)
	{
		...
		if (false === this.WordControl.m_oLogicDocument.Document_Is_SelectionLocked(AscCommon.changestype_Paragraph_Content, null, false)) {
			...
			if(_format == 1 && typeof data1 === 'string') {
				this._doRecordCollaboration(`插入内容: ${data1}`)
			} else if(data1 && data1.innerText) {
				this._doRecordCollaboration(`插入内容: ${data1.innerText}`)
			} else if(_format != 4) {
				this._doRecordCollaboration('执行了粘贴')
			}
		}
	};
	...
	asc_docs_api.prototype.addRowAbove             = function(count)
	{
		...
		if (doc.Document_Is_SelectionLocked(changestype_Drawing_Props, undefined, undefined, aAdditionalObjects) === false)
		{
			...
			this._doRecordCollaboration('表格：新增行')
		}
	};
	...
	asc_docs_api.prototype.addColumnLeft           = function(count)
	{
		...
		if (doc.Document_Is_SelectionLocked(changestype_Drawing_Props, undefined, undefined, aAdditionalObjects) === false)
		{
			...
			this._doRecordCollaboration('表格：新增列')
		}
	};
	...
	asc_docs_api.prototype.remRow                  = function()
	{
		...
		if (doc.Document_Is_SelectionLocked(changestype_Drawing_Props, undefined, undefined, aAdditionalObjects) === false)
		{
			...
			this._doRecordCollaboration('表格：删除行')
		}
	};
	asc_docs_api.prototype.remColumn               = function()
	{
		...
		if (doc.Document_Is_SelectionLocked(changestype_Drawing_Props, undefined, undefined, aAdditionalObjects) === false)
		{
			...
			this._doRecordCollaboration('表格：删除列')
		}
	};
	asc_docs_api.prototype.remTable                = function()
	{
		...
		if (doc.Document_Is_SelectionLocked(changestype_Drawing_Props) === false)
		{
			...
			this._doRecordCollaboration('删除表格')
		}
	};
	...
	asc_docs_api.prototype.asc_setViewMode        = function(isViewMode)
	{
		...
		this.WordControl.setNodesEnable(true);
		if (isViewMode)
		{
			this.ShowParaMarks          = false;
	...
	asc_docs_api.prototype.asc_addChartDrawingObject = function(chartBinary, Placeholder)
	{
		...
		if (AscFormat.isObject(chartBinary))
		{
			//if ( false === this.WordControl.m_oLogicDocument.Document_Is_SelectionLocked(changestype_Drawing_Props) )
			{
				...
				AscFonts.IsCheckSymbols = false;
				this._doRecordCollaboration('插入图表')
			}
		}
	};
	...
	asc_docs_api.prototype.onCInput = function(val, curPosInfo={})
	{
		if(val) {
			let actionCont = `插入内容：${val}`
			if(curPosInfo.curPage || curPosInfo.curPage == 0) {
				actionCont = `[页${curPosInfo.curPage + 1}]` + actionCont;
			}
	...
	asc_docs_api.prototype._doRecordCollaboration = function (content, actPos) {
		var Document = actPos || this.WordControl.m_oLogicDocument;
		let formatCont = content
		if(Document) {
			const curPage = Document.CurPage + 1
			formatCont = `[页${curPage}]${content}`
		}
		window['_PostActionContent'] && window['_PostActionContent'](formatCont)
	}
	...
	asc_docs_api.prototype["_doRecordCollaboration"] 		= asc_docs_api.prototype._doRecordCollaboration;
