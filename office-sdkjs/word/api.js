...
background-repeat: no-repeat;\
	...
	asc_docs_api.prototype.asc_PasteData = function(_format, data1, data2, text_data, useCurrentPoint, callback, checkLocks)
	{
		...
		if (isPasteImage) {
			...
			this._doRecordCollaboration('插入图片')
		} else {
			if(!text_data && data1 && !data1.startsWith('docData')) {
				this._doRecordCollaboration('粘贴：纯文本粘贴')
			} else {
				this._doRecordCollaboration('粘贴：带格式粘贴')
			}
		}
	...
	asc_docs_api.prototype.asc_RemoveTableCells    = function()
	{
		...
		if (!oLogicDocument.Document_Is_SelectionLocked(AscCommon.changestype_Remove))
		{
			...
			oLogicDocument.RemoveTableCells();
			oLogicDocument.FinalizeAction();
			this._doRecordCollaboration('表格：单元格左移')
			return true;
		}
		else
		{
	...
	{
		if(val && val != ' ') {
			if(curPosInfo.Type == 2) {
				window['_PostActionContent'] && window['_PostActionContent'](`在页眉/页脚插入内容：${val}`);
			} else {
				window['_PostActionContent'] && window['_PostActionContent'](`[页${curPosInfo.curPage + 1}]插入内容：${val}`);
			}
		}
	};
	...
	asc_docs_api.prototype._doRecordCollaboration = function (content, actPos) {
		...
		if(Document) {
			...
			//const curLine = (Document.CurPos && Document.CurPos.ContentPos || 0) + 1;
