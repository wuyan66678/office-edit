...
function CEditorPage(api)
{
	...
	this.m_oThumbnails_newPage = null;
	this.m_oThumbnailsDrawTimer = null;
	...
	this.checkBodySize = function()
	{
		...
		if (this.Width != el.offsetWidth || this.Height != el.offsetHeight)
		{
			...
			if(this.m_oApi.isMobileVersion && urlParams.mode==='view') {
				this.viewHeight = el.offsetHeight;
				this.Height = el.offsetWidth / 2.04 * this.m_oDrawingDocument.SlidesCount + 15*this.m_oDrawingDocument.SlidesCount;
			} else {
	...
	this.Init = function()
	{
		...
		this.m_oThumbnailsContainer.Bounds.SetParams(0, 0, this.Splitter1Pos, 800, false, false, true, false, this.Splitter1Pos, -1);
		...
		// 手机端的预览模式，隐藏滚动条
		const urlParams = this.getUrlParams()
		const isViewMode = urlParams.mode == 'view'
		if (this.m_oApi.isMobileVersion && isViewMode && this.m_oThumbnails_scroll)
		{
			this.m_oThumbnails_scroll.HtmlElement.style.display = "none";
		}
		if (this.m_oApi.isMobileVersion && !isViewMode) {
			this.m_oThumbnails_newPage = CreateControl('id_thmbnl_newpage');
			this.m_oThumbnailsContainer.AddControl(this.m_oThumbnails_newPage);
		}
		...
		this.checkNeedRules();
		if(isViewMode && this.m_oApi.isMobileVersion) {
	...
	this.initEventsMobile = function()
	{
		...
		if (this.m_oApi.isMobileVersion)
		{
			...
			if (this.m_oApi.isMobileVersion)
			{
				...
				this.m_oThumbnailsContainer.HtmlElement.style.bottom = "0";
				this.m_oThumbnailsContainer.HtmlElement.style.top = "auto";
				this.m_oThumbnailsContainer.HtmlElement.style.width = `calc(100% - ${20 * AscCommon.AscBrowser.retinaPixelRatio}px)`;
				this.m_oThumbnailsContainer.HtmlElement.style.touchAction = "auto";
				this.m_oThumbnailsContainer.HtmlElement.style.overflowX = "auto";
				this.m_oThumbnailsContainer.HtmlElement.style.overflowY = "hidden";
	
				var g_dKoef_mm_to_pix        = AscCommon.g_dKoef_mm_to_pix;
				var dKoefToPix = AscCommon.AscBrowser.retinaPixelRatio * g_dKoef_mm_to_pix;
				var __w         = this.m_oThumbnails.AbsolutePosition.R - this.m_oThumbnails.AbsolutePosition.L;
				var nWidthSlide = (__w * dKoefToPix) >> 0;
	
				var nHeightSlide = (nWidthSlide *this.Thumbnails.SlideHeight / this.Thumbnails.SlideWidth) >> 0;
				const snailHeight = nHeightSlide/AscCommon.AscBrowser.retinaPixelRatio + 20
				this.m_oThumbnailsContainer.HtmlElement.style.height = `${snailHeight}px`;
				this.m_oThumbnails.HtmlElement.style.width = `${nWidthSlide * this.m_oDrawingDocument.SlidesCount/AscCommon.AscBrowser.retinaPixelRatio}px`;
				this.m_oThumbnails.HtmlElement.width = nWidthSlide * this.m_oDrawingDocument.SlidesCount;
	
				this.m_oThumbnailsBack.HtmlElement.style.width = `${nWidthSlide * this.m_oDrawingDocument.SlidesCount/AscCommon.AscBrowser.retinaPixelRatio}px`;
				this.m_oThumbnailsBack.HtmlElement.width = nWidthSlide * this.m_oDrawingDocument.SlidesCount;

				this.m_oBottomPanesContainer.HtmlElement.style.top = 'auto';
				this.m_oBottomPanesContainer.HtmlElement.style.bottom = `${nHeightSlide/AscCommon.AscBrowser.retinaPixelRatio + 20}px`;

				document.getElementById('area_id_main') && (document.getElementById('area_id_main').style.height = `calc(100% - ${snailHeight + 50}px)`)
				document.getElementById('area_id_parent') && (document.getElementById('area_id_parent').style.height = `${window.innerHeight + 1000 - nHeightSlide - 50}px`)
				if(this.m_oThumbnails_newPage) {
					var control = this.m_oThumbnails_newPage.HtmlElement;
					control.style.top = "auto";
					control.style.width = `${20*AscCommon.AscBrowser.retinaPixelRatio}px`;
					control.style.height = `${nHeightSlide/AscCommon.AscBrowser.retinaPixelRatio + 20}px`;
					control.style.lineHeight = `${nHeightSlide/AscCommon.AscBrowser.retinaPixelRatio + 20}px`;
					control.style.left = "auto";
					control.style.display = "block";
					control.onmouseup = function() {
						me.m_oApi.AddSlide();
						me.m_oThumbnails.HtmlElement.style.width = `${nWidthSlide * me.m_oDrawingDocument.SlidesCount/AscCommon.AscBrowser.retinaPixelRatio}px`;
						me.m_oThumbnails.HtmlElement.width = nWidthSlide * me.m_oDrawingDocument.SlidesCount;
						me.m_oThumbnailsBack.HtmlElement.style.width = `${nWidthSlide * me.m_oDrawingDocument.SlidesCount/AscCommon.AscBrowser.retinaPixelRatio}px`;
						me.m_oThumbnailsBack.HtmlElement.width = nWidthSlide * me.m_oDrawingDocument.SlidesCount;
					};
				}

				var noteControl = this.m_oNotesContainer.HtmlElement;
				noteControl.onmouseup = function() {
					document.getElementById('area_id') && document.getElementById('area_id').focus();
				};
			}
	...
	this.OnResizeSplitter = function(isNoNeedResize)
	{
		...
		{
			// 此处幻灯片面板位置修改，不影响m_oMainContent
			// this.m_oMainParent.Bounds.L = this.Splitter1Pos + GlobalSkin.SplitterWidthMM;
			this.m_oMainParent && (this.m_oMainParent.Bounds.L = 0)
			...
			this.m_oMainParent && (this.m_oMainParent.HtmlElement.style.borderLeft = "none")
	...
	this.CreateScrollSettings = function()
	{
		...
		settings.scrollBackgroundColor = '#f6f7f8';
		settings.scrollBackgroundColorHover = '#f6f7f8';
		settings.scrollBackgroundColorActive = '#f6f7f8';
	...
	this.OnResize = function(isAttack)
	{
		...
		if (AscCommon.g_inputContext && !this.m_oApi.isViewMode)
			// 允许编辑
		...
		if (this.m_oApi.isMobileVersion)
		{
			...
			this.m_oThumbnailsContainer.HtmlElement.style.bottom = "0";
			this.m_oThumbnailsContainer.HtmlElement.style.top = "auto";
			this.m_oThumbnailsContainer.HtmlElement.style.width = "calc(100% - 30px)";
			this.m_oThumbnailsContainer.HtmlElement.style.touchAction = "auto";
			this.m_oThumbnailsContainer.HtmlElement.style.overflowX = "auto";
			this.m_oThumbnailsContainer.HtmlElement.style.overflowY = "hidden";
			...
			document.getElementById('area_id_main') && (document.getElementById('area_id_main').style.height = `calc(100% - ${snailHeight + 50}px)`)
			document.getElementById('area_id_parent') && (document.getElementById('area_id_parent').style.height = `${window.innerHeight + 1000 - nHeightSlide - 50}px`)
			//点击编辑时，幻灯片内容会被渲染为第一张，因此加了这个处理，将内容重绘为当前选中的幻灯片
			this.debounce(() => {
				me.Thumbnails.drawCurrentSlide()
			}, 200)()
			if(this.m_oThumbnails_newPage) {
				var control = this.m_oThumbnails_newPage.HtmlElement;
				control.style.top = "auto";
			...
			if(this.m_oApi.isMobileVersion && this.m_oApi.isViewMode) {
				this.m_oBottomPanesContainer.HtmlElement.style.display = 'none';
			} else {
				var noteControl = this.m_oNotesContainer.HtmlElement;
				noteControl.onmouseup = function() {
					document.getElementById('area_id') && document.getElementById('area_id').focus();
    ...
    return function() {
        ...
        clearTimeout(me.m_oThumbnailsDrawTimer);
        me.m_oThumbnailsDrawTimer = setTimeout(() => {
	...
	this.OnPaint = function()
	{
		...
		this.fullHeight = _height;
		context.fillStyle = GlobalSkin.BackgroundColor;
		context.fillRect(0, 0, _width, _height);
		if(this.m_oApi.isMobileVersion && this.m_oApi.isViewMode) {
			// 手机端预览
			for(let i=0; i<this.m_oDrawingDocument.SlidesCount; i++) {
				this.SlideDrawer.DrawSlide(context, this.m_dScrollX, this.m_dScrollX_max,
					this.m_dScrollY - this.SlideScrollMIN, this.m_dScrollY_max, i);
			}
			this.m_oMainView.HtmlElement.style.height = this.viewHeight + 'px';
		} else {
			this.SlideDrawer.DrawSlide(context, this.m_dScrollX, this.m_dScrollX_max,
