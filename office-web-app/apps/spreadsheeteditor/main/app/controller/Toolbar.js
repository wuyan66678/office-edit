...
define([
    ...
    SSE.Controllers.Toolbar = Backbone.Controller.extend(_.extend({
        ...
        initialize: function() {
            ...
            this.onApiEndAddShape = function() {
                if (this.toolbar.btnInsertShape.pressed){
									...
									window['_PostActionContent'] && window['_PostActionContent']('插入形状');
								}
                if (this.toolbar.btnInsertText.pressed) {
									this.toolbar.btnInsertText.toggle(false, true);
									window['_PostActionContent'] && window['_PostActionContent']('插入文本框');
								}
                $(document.body).off('mouseup', checkInsertAutoshape);
            };
        },
        ...
        onSearch: function(btn, s){
            ...
            // var mode = btn.options.action
            var mode = btn ? btn.options.action : s

            // if(this.dlgSearch['hide']){
        ...
        setApi: function(api) {
            ...
            if(!config.isEdit) {
                //改造-- 预览模式也可筛选
                this.api.asc_registerCallback('asc_onSelectionChanged',         _.bind(this.onApiSelectionChangedLFixed, this));
...
//            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
        ...
        onBold: function(btn, e) {
            ...
            if (this.api)
                this.api.asc_setCellBold(btn.pressed);
						if(btn.pressed) {
							this.postActionContent(`的格式 字体：加粗`)
						} else {
							this.postActionContent(`的格式 字体：取消加粗`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
        ...
        onItalic: function(btn, e) {
            ...
            if (this.api)
                this.api.asc_setCellItalic(btn.pressed);
						if(btn.pressed) {
							this.postActionContent(`的格式 字体：倾斜`)
						} else {
							this.postActionContent(`的格式 字体：取消倾斜`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
        ...
        onUnderline: function(btn, e) {
            ...
            if (this.api)
                this.api.asc_setCellUnderline(btn.pressed);
						if(btn.pressed) {
							this.postActionContent(`的格式 字体：下划线`)
						} else {
							this.postActionContent(`的格式 字体：取消下划线`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
        ...
        onStrikeout: function(btn, e) {
            ...
            if (this.api)
                this.api.asc_setCellStrikeout(btn.pressed);
						if(btn.pressed) {
							this.postActionContent(`的格式 字体：删除线`)
						} else {
							this.postActionContent(`的格式 字体：取消删除线`)
						}
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
        ...
        onAutoFontColor: function(e) {
            ...
            Common.component.Analytics.trackEvent('ToolBar', 'Text Color');
				...
				postActionContent: function (type, prefix='设置') {
					try {
						const content = this.api.formatSelectedCells(type, prefix)
						if(content) {
        ...
        onBordersMenu: function(menu, item) {
            ...
            if (me.api && !_.isUndefined(item.options.borderId)) {
                ...
                Common.component.Analytics.trackEvent('ToolBar', 'Borders');
								this.postActionContent(`的格式：边框：${item.caption}`)
            }
        },
            ...
            if (this.api) {
                ...
                this.toolbar.btnWrap.allowDepress = !(type == AscCommon.align_Justify);
						...
						if(content) {
							...
							this.postActionContent(`的格式 对齐：${content}`)
						}
        ...
        onMergeCellsMenu: function(menu, item) {
            ...
            function doMergeCells(how) {
                ...
                Common.component.Analytics.trackEvent('ToolBar', 'Merge');
								if($('#id-toolbar-rtn-merge').hasClass('active')) {
									me.postActionContent('', '合并单元格')
								} else {
									me.postActionContent('', '取消合并单元格')
								}
            }
        ...
        onTextOrientationMenu: function(menu, item) {
            ...
            Common.component.Analytics.trackEvent('ToolBar', 'Text orientation');
						// todo---
						this.postActionContent(`格式：方向：${item.caption}`)
        },
                    ...
                    switch (res) {
                        ...
                        case Asc.c_oAscSelectionSortExpand.notExpandAndNotShowMessage:
                            ...
                            break;
								...
								this.postActionContent(actionCont, '对')
            }
        },
            ...
            if (record) {
                if (this.api)
                    this.api.asc_setCellFormat(record.format);
								this.postActionContent(`的格式：数字格式：${record.displayValue}`)
            } else {
                this.onCustomNumberFormat();
            }
            Common.NotificationCenter.trigger('edit:complete', this.toolbar);
            ...
            if (this.api) {
                if (record.isNewFont) {
                    ...
                    Common.component.Analytics.trackEvent('ToolBar', 'Font Name');
								...
								this.postActionContent(`的格式：字体：${record.name}`)
            }
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
        },
        ...
        onFontSizeSelect: function(combo, record) {
            ...
            if (this.api)
                this.api.asc_setCellFontSize(record.value);
						this.postActionContent(`的格式：字体大小：${record.value}pt`)
            Common.NotificationCenter.trigger('edit:complete', this.toolbar, {restorefocus:true});
            Common.component.Analytics.trackEvent('ToolBar', 'Font Size');
        },
                ...
                picker.on('item:click', function(picker, item, record) {
                    if (me.api) {
                        ...
                        Common.component.Analytics.trackEvent('ToolBar', 'Table Templates');
												me.postActionContent(`设置表格样式：${record.attributes && record.attributes.tip}`, '对')
                    }
                });
...
//                (need_disable !== toolbar.btnMerge.isDisabled()) && toolbar.btnMerge.setDisabled(need_disable);
        ...
        //改造-- 预览模式下方法
        onApiSelectionChangedLFixed: function(info) {
            if (!this.editMode || $('.asc-window.enable-key-events:visible').length>0) return;
            var toolbar = this.toolbar
        ...
        onAppShowed: function (config) {
            ...
            if ( config.isEdit ) {
                ...
                if ( !config.isEditDiagram && !config.isEditMailMerge ) {
                    ...
                    if ($panel) {
                        config.canProtect && $panel.append($('<div class="divider"></div>'));
                ...
                if( !config.isEditDiagram && !config.isEditMailMerge) {
                    ...
                    Array.prototype.push.apply(me.toolbar.lockControls, datatab.getButtons());
                    me.toolbar.btnsSetAutofilter = datatab.getButtons('set-filter');
                    me.toolbar.btnsSetAutofilter.setDisabled(false)
