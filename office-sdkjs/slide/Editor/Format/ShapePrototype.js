            ...
            if(hierarchy[0])
            {
                ...
                if(doc_content)
                {
                    ...
                    doc_content.SetApplyToAll(false);
										actionContent = '删除占位符内容'
                }
            }
        }
				if(bCheckPlaceholder) {
					const curPage = editor.WordControl && editor.WordControl.m_oLogicDocument && editor.WordControl.m_oLogicDocument.CurPage || 0
					window['_PostActionContent'] && window['_PostActionContent'](`[页${curPage + 1}]${actionContent}`)
				}
        return pos;
    }
    return -1;
