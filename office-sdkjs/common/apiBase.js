...
(function(window, undefined)
{
	...
	baseEditorsApi.prototype.asc_updateDocTitle                  = function(value)
	{
		this.DocInfo.put_Title(value);
	...
	baseEditorsApi.prototype.onDocumentContentReady              = function()
	{
		...
		//设置定时器，24（86400000）小时后提醒用户关闭页面重新打开
		/* setTimeout(() => {
			t.sendEvent('asc_onError', c_oAscError.ID.PageExpired, c_oAscError.Level.NoCritical, '您已访问此页面超过24小时，为保障数据安全，请关闭页面重新打开！');
		}, 3000) */

		// 获取页面加载时间
		const pageLoadTime = new Date().getTime();
		const pageLoadDate = new Date(pageLoadTime).toDateString(); // 保存页面加载日期

		// 设置检查间隔时间（比如每分钟检查一次）
		const checkInterval = 30 * 60 * 1000; // 30分钟（单位为毫秒）
		function checkMidnightCrossed() {
				const now = new Date();
				const currentDate = now.toDateString();
				// 如果日期不同，说明已经跨过午夜,提醒用户关闭页面重新打开
				if (currentDate !== pageLoadDate) {
					t.sendEvent('asc_onError', c_oAscError.ID.PageExpired, c_oAscError.Level.NoCritical, '您已访问此页面超过24小时，为保障数据安全，请关闭页面重新打开！');
					// 停止进一步的检查
		...
		// 设置定时检查
		const intervalId = setInterval(checkMidnightCrossed, checkInterval);
	};
	// Save
	...
	baseEditorsApi.prototype.AddImageUrl       = function(urls, imgProp, token, obj)
	{
		...
		var callback = function(urls) {
			...
			t.sendEvent("asc_onImgUploadSuccess");
		};
	...
	baseEditorsApi.prototype._uploadCallback                     = function(error, files, obj)
	{
		...
		else
		{
			...
			AscCommon.UploadImageFiles(files, this.documentId, this.documentUserId, this.CoAuthoringApi.get_jwt(), function(error, urls)
			{
				...
				{
					t._addImageUrl(urls, obj);
					t.sendEvent("asc_onImgUploadSuccess", obj);
				}
				t.sync_EndAction(c_oAscAsyncActionType.BlockInteraction, c_oAscAsyncAction.UploadImage);
			});
		}
