...
(function (window, undefined) {
		...
		CCollaborativeEditing.prototype.lock = function (arrLocks, callback) {
			...
			//改造--   预览模式下可筛选
			!this.m_bIsViewerMode && this.onStartCheckLock();
			...
			!this.m_bIsViewerMode && this.onEndCheckLock(callback);
			return true;
		};
		...
		CCollaborativeEditing.prototype.onEndCheckLock = function (callback) {
			...
			if (this.m_arrCheckLocks.length > 0) {
				// Отправляем запрос на сервер со списком элементов (发生一个带有元素列表的服务器请求)
				this.handlers.trigger("askLock", this.m_arrCheckLocks, function (result) {t.onCallbackAskLock (result, callback);});

				if (undefined !== callback) {
					// Ставим глобальный лок (только если мы не одни и ждем ответа!) 设置全球锁(除非我们不是唯一在等待答案的人!)
				...
				// Снимаем глобальный лок (для редактирования ячейки) 拍摄全球锁(用于编辑单元格)
				this.m_bGlobalLockEditCell = false;
			}
		};
		...
		CCollaborativeEditing.prototype.sendChanges = function (IsUserSave, isAfterAskSave) {
			//当没有联合编辑的时候，你不需要清理任何东西，但你需要发送任何东西。
			...
			AscCommon.History.SaveChangeLFixed()
			// Отправляем на сервер изменения 将更改结果发送到服务器
			...
			if (bIsCollaborative) {
				...
				// Чистим Undo/Redo 清除  改造--取消历史清除？(本地编辑在未有新保存时可撤回，协同时可取消/重做)
				// AscCommon.History.Clear();
				
				AscCommon.History.ClearLFixed()
				// Перерисовываем
				if (bCheckRedraw) {
...
//                }
		...
		//改造-- 仅删除别人的锁
		CCollaborativeEditing.prototype.clearLockLFixed = function (IsUserSave, isAfterAskSave) {
			// 当没有联合编辑的时候，你不需要清理任何东西，但你需要发送任何东西。
			// Когда не совместное редактирование чистить ничего не нужно, но отправлять нужно.
			var bIsCollaborative = this.getCollaborativeEditing();
			...
			if (bIsCollaborative) {
				...
				// Очищаем свои изменения 清理自己的变化
				while (0 < this.m_arrNeedUnlock2.length) {
					oLock = this.m_arrNeedUnlock2.shift();
					oLock.setType(c_oAscLockTypes.kLockTypeNone, false);
			...
			if (bIsCollaborative) {
				// Пересчитываем lock-и от чужих пользователей 从外部用户数lock。
				this._recalcLockArrayOthers();
				...
				this.m_oInsertColumns = {};
				this.m_oInsertRows = {};
				// Очищаем свои пересчетные индексы 清理你的计数指数
				this.clearRecalcIndex();
...
//                }
				...
				// Обновляем точку последнего сохранения в истории 更新历史上最后一个保存点
				AscCommon.History.Reset_SavedIndex(IsUserSave);
			}
		};
		...
		/**
		 * Проверка lock для элемента 元件锁检验
		 * @param {asc_CLockInfo} element  элемент для проверки lock 锁检验元件
		...
		CCollaborativeEditing.prototype.getLockIntersection = function (element, type, bCheckOnlyLockAll) {
			...
			for (var i = 0; i < arrayElements.length; ++i) {
				...
				if (c_oAscLockTypeElem.Sheet === element["type"] && element["type"] === oUnlockElement["type"]) {
					...
					//只有删除页面检查(如果是自己检查的话，我们不会马上离开，因为我们需要检查其他元素的锁)
				...
				// 不包括changeproties lock(除非是lock list)
		...
		CCollaborativeEditing.prototype.removeColsRange = function (sheetId, range) {
			...
			for (var i = 0; i < arrInsertColumns.length; ++i) {
				if (arrInsertColumns[i].c1 > range.c2) {
					// Справа от удаляемого диапазона 远距范围右
					arrInsertColumns[i].c1 -= countCols;
					arrInsertColumns[i].c2 -= countCols;
				} else if (arrInsertColumns[i].c1 >= range.c1 && arrInsertColumns[i].c2 <= range.c2) {
					// Полностью включение в удаляемый диапазон 远距离全接合
					...
					// Частичное включение начала диапазона 部分波段耦合
					...
					// Частичное включение окончания диапазона 部分波段耦合
					arrInsertColumns[i].c2 = range.c1 - 1;
				} else if (arrInsertColumns[i].c1 < range.c1 && arrInsertColumns[i].c2 > range.c2) {
					// Удаляемый диапазон внутри нашего диапазона 我们范围内的远程范围
					arrInsertColumns[i].c2 -= countCols;
				}
			}
		};
