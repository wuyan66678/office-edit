...
DrawingObjectsController.prototype.addImageFromParams = function(rasterImageId, x, y, extX, extY)
{
    var image = this.createImage(rasterImageId, x, y, extX, extY);
		var curSheet = this.drawingObjects.getWorksheetModel()
    image.setWorksheet(curSheet);
    ...
    image.addToRecalculate();
		window['_PostActionContent'] && window['_PostActionContent'](`[${curSheet.sName}(x:${Math.ceil(x)},y:${Math.ceil(y)})]插入图片`)
};
