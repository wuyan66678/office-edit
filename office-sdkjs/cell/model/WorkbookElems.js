...
function RangeDataManagerElem(bbox, data)
{
	...
	AutoFilter.prototype.setRowHidden = function(worksheet, newFilterColumn, opt_columnsFilter) {
		...
		//改造-- 记录上次过滤行信息
		var lastHiddenObj = {start: this.Ref.r1+1, h: null}
		...
		for (var i = startRow; i <= endRow; i++) {
			...
			if(!newFilterColumn) {
				...
				if (!isHidden) {
					...
					//改造--记录上次过滤数据
					var rowLastHidden = worksheet.getRowHidden(i);
					if(lastHiddenObj.h === null){
						lastHiddenObj.h = rowLastHidden;
						lastHiddenObj.start = i
					} else if(lastHiddenObj.h !== rowLastHidden){
						worksheet.setLastRowHiddenLFixed(lastHiddenObj.h, lastHiddenObj.start, i-1);
						lastHiddenObj.h = rowLastHidden;
						lastHiddenObj.start = i
					}
					if (i === endRow) {
						worksheet.setLastRowHiddenLFixed(lastHiddenObj.h, lastHiddenObj.start, i);
					}
