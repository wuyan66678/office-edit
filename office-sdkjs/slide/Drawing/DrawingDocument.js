...
function CThumbnailsManager()
{
	...
	this.ConvertCoords  = function(x, y, isPage, isFixed)
	{
		...
		Pos.Y = AscCommon.AscBrowser.convertToRetinaValue(Pos.Y, true) - 98;//修改样式后幻灯片偏移
	...
	this.ConvertCoords2 = function(x, y)
	{
		...
		Pos.X = AscCommon.AscBrowser.convertToRetinaValue(Pos.X, true) + 391;//修改样式后幻灯片偏移
		Pos.Y = AscCommon.AscBrowser.convertToRetinaValue(Pos.Y, true)- 98;//修改样式后幻灯片偏移
	...
	this.onMouseUp = function(e, bIsWindow)
	{
		...
		if(oThis.m_oWordControl.m_oApi.isMobileVersion) {
			var canvas = oThis.m_oWordControl.m_oEditor.HtmlElement;
			...
			var word_control = oThis.m_oWordControl;
			...
			nWidthSlide -= (oThis.const_offset_x + oThis.const_offset_r);
			var pointPage = Math.floor(e.offsetX / ((nWidthSlide + 3 * oThis.const_border_w)/AscCommon.AscBrowser.retinaPixelRatio))
			oThis.m_oWordControl.m_oDrawingDocument.SlideCurrent = pointPage
			oThis.m_oWordControl.SlideDrawer.DrawSlide(context, null, null, null, null, oThis.m_oWordControl.m_oDrawingDocument.SlideCurrent, true);
			oThis.m_oWordControl.m_oLogicDocument.Set_CurPage(pointPage)
			for(let i=0; i<oThis.m_arrPages.length; i++) {
				oThis.m_arrPages[i].IsSelected = false
			}
			oThis.m_arrPages[pointPage] && (oThis.m_arrPages[pointPage].IsSelected = true);
			oThis.OnUpdateOverlay();
			oThis.needReDraw = true;
			// 点击会触发弹起键盘，所以加了这行
			document.getElementById('area_id') && document.getElementById('area_id').blur();
		}
		...
		if (oThis.IsMouseDownTrackSimple)
		{
			...
			// послали уже на mouseDown 已发送到mouseDown
			// oThis.SelectPageEnabled = false;
		...
		else
		{
			...
			{
				// вызвать функцию апи для смены слайдов местами //更改幻灯片位置
	...
	this.drawCurrentSlide = function () {
		if(this.needReDraw) {
			var canvas = oThis.m_oWordControl.m_oEditor.HtmlElement;
			var context = canvas.getContext("2d");
			this.m_oWordControl.SlideDrawer.DrawSlide(context, null, null, null, null, this.m_oWordControl.m_oDrawingDocument.SlideCurrent, true);
		}
	}
	...
	this.CheckSizes = function()
	{
		...
		{
			// все убралось. скролл не нужен 不需要滚动条
		...
		else
		{
			...
			// теперь нужно выставить размеры 设定尺寸
	...
	this.CalculatePlaces = function()
	{
		...
		var leftStart = this.const_offset_x;
		for (var i = 0; i < this.SlidesCount; i++)
		{
			var leftCurrentInDoc = nWidthSlide * i + 3 * this.const_border_w*i;
	...
	this.OnPaint = function()
	{
		...
		var context = canvas.getContext("2d");
		// var _width  = canvas.width;
		var _width  = 230 * this.SlidesCount;
		var _height = canvas.height;
		...
		for (var i = 0; i < this.SlidesCount; i++)
		{
			...
			if(this.m_oWordControl.m_oApi.isMobileVersion) {
				page.Draw(context, page.left, page.originTop, page.right - page.left, page.bottom - page.top);
			} else {
...
function CSlideDrawer()
{
	...
	this.DrawSlide = function(outputCtx, scrollX, scrollX_max, scrollY, scrollY_max, slideNum, noCache=false)
	{
		if (this.m_oWordControl.m_oApi.isReporterMode)
			return;
		...
		if(this.m_oWordControl.m_oApi.isMobileVersion && this.m_oWordControl.m_oApi.isViewMode) {
			// app端预览模式
			var dKoef = this.m_oWordControl.m_nZoomValue * g_dKoef_mm_to_pix / 100;
			dKoef *= AscCommon.AscBrowser.retinaPixelRatio;
			...
			var g = new AscCommon.CGraphics();
			if(this.m_oWordControl.m_oApi.isMobileVersion) {
				g.init(outputCtx, w_px, h_px, w_mm, h_mm + 15);
			} else {
			...
			g.m_oCoordTransform.ty = slideNum * h_px;
			g.transform(1, 0, 0, 1, 0, 0);
