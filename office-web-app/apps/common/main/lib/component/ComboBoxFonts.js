        ...
        return {
            ...
            render : function(parentEl) {
                ...
                this._input.on('blur',      _.bind(function() {this.inFormControl = false;}, this));
								...
								if(this.fixedPosition && menuRoot){
									menuRoot.css({
										position    : 'fixed',
										right       : 'auto',
										left        : -1000,
            ...
            loadSprite: function(callback) {
                ...
                this.spriteThumbs.load(thumbs[thumbIdx].path, callback);
						 ...
						 isChineseCharacter: function(char) {
							// 使用正则表达式匹配中文汉字（包括扩展汉字）
							const regex = /^[\u4e00-\u9fa5\u9fa6-\u9fff]+$/;
							return regex.test(char);
						},
            fillFonts: function(store, select) {
							var me = this;
							const arrStore = store.toJSON()
							arrStore.sort((a,b)=>{
								const nameA = a.name.toLowerCase(); // 转换为小写以实现不区分大小写排序
								const nameB = b.name.toLowerCase();
								if(this.isChineseCharacter(nameA[0]) && !this.isChineseCharacter(nameB[0])) {
									return -1
								} else if(!this.isChineseCharacter(nameA[0]) && this.isChineseCharacter(nameB[0])) {
									return 1
								}
								if (nameA < nameB) {
									return 1; // nameB 在前
